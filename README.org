...
*** 🔄 Smoother Undo
Previously, undoing a change forced a full page reload. Now:
- Undo reloads the last saved snapshot *in place*, without disrupting navigation.
- The calendar is re-rendered using ~loadCalendarAroundDate(todayDate)~, keeping your scroll position intact.
- This makes undoing changes faster and more seamless.

*** ✨ Highlight "Clicked" Day
To provide feedback when adding a note:
- The clicked day briefly flashes with a highlight effect (~.clicked-day~ class).
- The highlight disappears after 500ms, confirming your action.

*** 📝 Smarter Multi-Line Input
Writing multi-line notes is now easier:
- *Enter* (without Shift) saves and closes the note.
- *Shift+Enter* inserts a new line, allowing bullet points or longer text blocks.
- This improves text editing without disrupting quick note-taking.

*** 📅 Time Range for Date Picker
The "Jump to Date" field now includes a default range:
~<input type="date" id="jumpDate" min="2000-01-01" max="2050-12-31">~
- This ensures practical usability while allowing easy adjustments if needed.

** 🎨 Cosmetic & UI Improvements

*** 📏 Improved Header Spacing
- We *removed excess padding* from the header and instead applied:
  ```css
  #calendarContainer {
    margin-top: 100px;
  }
