...
[[https://timeless-calendar.vercel.app/][Timeless: The Infinitely Scrolling Calendar 🎡🕰️]]
...
** Summary:
- A web-based, infinitely scrolling calendar built with React and TypeScript that lets you add notes to any day. Desktop and mobile views use virtualization for smooth performance across thousands of days. 🔄📖
- Data persists immediately to ~localStorage~ and syncs in the background to your Dropbox account via ~timeline.md~, ensuring privacy and cross-device access. 🔐📝
...
- Export your calendar as a Markdown diary (~Cmd/Ctrl+E~) or JSON backup. Import from JSON to restore or migrate data. 💾📂
- Background sync uses Dropbox file revisions: a stale browser cannot overwrite a newer diary, and Timeless asks which copy to keep when both changed.
   + Health checks available via ~npm run health~ to verify sync integrity. 🔄🌐
...
* Getting Started
** Non-expert users
Simply visit: [[https://timeless-calendar.vercel.app/][Timeless: The Infinitely Scrolling Calendar 🌏🚀]]
...
- *Revision-guarded writes* - Saves update only the exact Dropbox revision the browser last synchronized; stale writes return HTTP 409
...
** Cloud Sync (Dropbox)
- *Background sync:* Changes sync to the configured Dropbox diary path; production uses ~/github/timeless/data/timeline.md~
- *Revision baseline:* Each load records Dropbox’s current file revision, and each save must match it
- *Markdown format:* Uses human-readable Markdown for version control and easy inspection
- *Conflict resolution:* A revision mismatch preserves browser edits and offers explicit ~Use Dropbox~ and ~Keep Browser~ choices; timestamps are retained for display and diagnostics, not as a lock
...
- *Health checks:* Run ~npm run health~ to validate the production load endpoint, Dropbox credentials, timestamp, and populated-date count
...
- *Type definitions:* ~src/types/index.ts~ - TypeScript interfaces for events and calendar data 🎨🔧
