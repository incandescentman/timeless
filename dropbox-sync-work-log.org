...
### Fixed - 2025-10-15
- **Mobile Footer Safe Area:** Implemented a robust fix for the mobile footer being cut off on iPhones. The footer is now lifted above the safe area inset, with a pseudo-element extending the background to the screen edge for a seamless look.

### Fixed - 2025-10-15
- **True Root Cause Identified:** The mobile sync issue was caused by a stale Dropbox refresh token on the Vercel server. The API was failing to fetch data from Dropbox and returning an empty response. The fix was to update the `DROPBOX_REFRESH_TOKEN` environment variable on Vercel.

### Changed - 2025-10-15
- Attempted to fix a mobile sync issue by removing `Date.now()` from the `calendar-load` API's timestamp calculation. This ensures the server reports the true file modification time, but the underlying sync problem on mobile persists and requires further investigation.
...
- Verified production flow: web captures write through to `data/timeline.md` in Dropbox (renamed from `jay-diary.md`). Manual edits now piggyback on Dropbox `server_modified` timestamps (no manual comment bump); next test is remote edit → auto pull in browser.
