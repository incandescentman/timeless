...
- Added a layout-priming pass that precomputes the initial offset using the virtual month heights, applies it to both `window.scrollTo` and the virtualizer state, so we render today’s month before any follow-up retries.
