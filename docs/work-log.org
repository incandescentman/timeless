...
- This makes the API response idempotent for an unchanged file, allowing clients to correctly determine when they need to update their local `localStorage` cache.
...
Benefits:
- Restores reliable, timestamp-based data synchronization.
- Fixes the issue where mobile clients were not seeing updated events.
