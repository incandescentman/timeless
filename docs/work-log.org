...
- Reinstated the primary `MobileEventComposer` so the UI relies on a single code path again
...
** Session: Remove Modal Keyboard Offset Experiment
Time: Late evening
Task: Roll back native-dialog/keyboard-offset logic per request
Status: **COMPLETED**

Accomplished:
- Simplified `MobileEventComposer` to the original overlay-only implementation
- Removed visualViewport keyboard offset calculations and native `<dialog>` fallback code, returning to the straightforward portal overlay
- Pruned unused dialog-related CSS to keep styles lean

Lessons Learned:
- Native dialog support introduced extra branches without clear UX win; simpler overlay matches expected feel
- Keyboard offset juggling added maintenance cost and visual artifacts when the keyboard dismissed mid-save

Next Step:
- Re-test mobile capture flow to confirm saves complete without layout shifts or lingering drafts
