...
Solution:
- Refactored `MobileEventComposer` to auto-focus the single input and funnel close events through a `commitAndClose` helper that saves non-empty drafts and otherwise closes silently.
- Removed footer button row; outside taps, Escape key, and backdrop clicks now either save or dismiss based on content.
- Restyled the modal to float mid-screen with centered metadata + hint copy so it visually mirrors the canonical minimalist card while respecting Timeless color tokens.
