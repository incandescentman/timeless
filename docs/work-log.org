...
Next Step: Add a lightweight health check that alerts when `/api/calendar-load` starts returning token errors so we catch credential drift before clients notice.
