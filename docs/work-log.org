...
Problem:
The user reported that edits made directly to the `jay-diary.md` file were visible on the web app on a laptop but not on an iPhone. This pointed to a client-side caching or data invalidation problem specific to mobile.
...
Problem:
- Mac client showed fresh events from `data/jay-diary.md`, but iPhone rendered an empty calendar despite a successful save.
