...
** Session: Final Resolution of Mobile Sync Issue
Time: Afternoon
Task: Document the true root cause and solution for the mobile sync failure.
Status: **COMPLETED**

Problem:
After a previous attempted fix, the user reported that the mobile sync issue persisted. The iPhone client was still not displaying the latest calendar events.

Root Cause Analysis:
The issue was not client-side logic, but server-side authentication. The Vercel deployment was using a stale or invalid Dropbox refresh token. This caused the `/api/calendar-load` function to fail when trying to download the diary file from Dropbox.

- The API would then return an empty calendar response to the client.
- Desktop browsers appeared to work correctly because they were relying on a recent, valid cache of events in `localStorage`.
- Mobile browsers, often starting with a clear cache, would fetch the empty response from the failing API and display a blank calendar.

Solution:
The user manually updated the Dropbox API credentials (specifically the `DROPBOX_REFRESH_TOKEN`) in the Vercel project's environment variables. Once the server had a valid token, it could successfully authenticate with Dropbox, fetch the diary file, and serve the correct data to all clients.

Key Takeaway:
When debugging sync issues, always verify the entire data path, including server-side authentication with third-party services. Client-side caches can easily mask upstream failures.

** Session: Fix Mobile Sync Issue
Time: Afternoon
Task: Diagnose and fix an issue where diary changes were not appearing on iPhone.
Status: **IN PROGRESS**

Problem:
The user reported that edits made directly to the `jay-diary.md` file (now renamed to `timeline.md`) were visible on the web app on a laptop but not on an iPhone. This pointed to a client-side caching or data invalidation problem specific to mobile.

Root Cause Analysis:
The serverless function `api/calendar-load.js` was including the current server time (`Date.now()`) in the `lastSavedTimestamp` it sent to the client. This meant every API response had a brand new, unique timestamp, regardless of whether the underlying diary file had actually changed. This broke the timestamp-based synchronization logic.

Attempted Solution:
- Removed the use of `Date.now()` from the `Math.max()` calculation in `api/calendar-load.js`.
- The server now only uses the timestamp embedded in the diary file or the `server_modified` timestamp from the Dropbox file metadata.
- This was intended to make the API response idempotent for an unchanged file, allowing clients to correctly determine when they need to update their local cache.

Result:
The fix was deployed, but the user reports the issue is still occurring on iPhone. The initial diagnosis was correct but insufficient. The root cause is not fully addressed, and further investigation into the client-side fetching and caching logic is required.

Files Changed:
- `api/calendar-load.js` (fixed timestamp logic)
- `dropbox-sync-work-log.org` (updated changelog)
- `docs/work-log.org` (this entry)

Benefits:
- The server-side API now correctly reports the file modification timestamp, which is a necessary step for reliable sync.
...
Accomplished:
- Reworked `exportAsMarkdownDiary()` to emit year and month headings plus `M/D/YYYY` lines with indented bullet notes so exports align with `timeline.md` (formerly `jay-diary.md`).
- Normalised event entries to preserve completion ticks and tags while skipping empty rows.
- Renamed the download target to `jay-diary.md` (now updated again to `timeline.md`) to match the repository file and avoid date-stamped filenames.
...
** Session: Diary rename + env sync
Time: Evening
Task: Align codebase and infrastructure with `jay-diary.md` → `timeline.md` rename

Accomplished:
- Updated dev middleware, Dropbox API handlers, and markdown export download name to target `timeline.md`.
- Refreshed instructions, migration notes, Emacs guide, and related documentation to reflect the new canonical file name.
- Coordinated with production by updating `DROPBOX_CALENDAR_PATH` to `/github/timeless/data/timeline.md` so Vercel now reads and writes the renamed diary.

Energy Level: Intentional
Next Step: Monitor upcoming sync health checks to confirm the renamed path continues to round-trip cleanly across devices.

** Session: Dropbox diary sync repair
Time: Evening
Task: Restore calendar updates across devices after mobile stopped receiving new entries

Problem:
- Mac client showed fresh events from `data/jay-diary.md` (now `data/timeline.md`), but iPhone rendered an empty calendar despite a successful save.

Root Cause:
- `/api/calendar-load` returned `invalid_grant` from Dropbox because the Vercel `DROPBOX_REFRESH_TOKEN` had gone stale. Desktop still looked correct thanks to cached localStorage; mobile had no cache, so the failed fetch left it blank.

Solution:
- Generated a new refresh token for the Dropbox app, updated `DROPBOX_APP_KEY`/`DROPBOX_APP_SECRET`/`DROPBOX_REFRESH_TOKEN` in Vercel, and redeployed so the refresh flow could mint a fresh access token.
- Verified `https://timeless-calendar.vercel.app/api/calendar-load` returns JSON and confirmed both desktop and iPhone load the same events after a hard refresh.
- Added `scripts/check-calendar-health.mjs` plus `npm run health` to quickly detect future token expiries without waiting for client reports.
- Applied `viewport-fit=cover` and safe-area padding adjustments to lift the mobile footer, though QA shows the buttons still grazed the iPhone home indicator.
- Escalated the mobile footer fix by offsetting the bar above the safe-area inset and extending a backdrop pseudo-element so buttons no longer clip against the iPhone’s curved corners.
- Traced the red-on-red “Today” event styling to mobile overrides and captured the fix so today’s notes stay legible on the scarlet cell.

Energy Level: Relieved
Next Step: Automate the new health check (Scheduled GitHub Action or external cron) so token failures trigger alerts without manual runs.
Handoff: Schedule recurring `npm run health` (GitHub Action or external cron) and keep validating the mobile footer on physical devices to ensure the lifted bar stays clear of curved corners across models.

** Session: Mobile footer height correction
Time: Late night
Task: Lower the safe-area offset so mobile controls sit closer to the home indicator

Accomplished:
- Re-anchored the footer to `bottom: 0` and trimmed its minimum height so navigation buttons rest just above the iOS home affordance (`src/styles/mobile-footer.css`).
- Realigned the pseudo-background extension and calendar bottom margin to preserve the glass backdrop while keeping the grid clear of the footer (`src/styles/mobile-footer.css`).

Energy Level: Grounded
Next Step: Validate on physical iOS and Android devices that the adjusted offset feels natural yet still clears the system gesture zone.
Handoff: Capture feedback from real device smoke-tests (screenshot + notes) and adjust safe-area padding if thumbs report cramped spacing near the gesture bar.

** Session: Mobile footer spacing tune
Time: Night
Task: Dial in footer button position and tighten mobile event density

Accomplished:
- Iterated the button offset to land at a 9px relative drop, keeping the frosted tray fixed while restoring thumb clearance (`src/styles/mobile-footer.css`).
- Trimmed the mobile event card vertical padding so stacked notes fit without overwhelming shorter viewports (`src/styles/mobile.css`).

Energy Level: Focused
Next Step: Run the latest build on physical iOS/Android hardware to confirm the footer alignment feels natural and the denser event stack stays legible.
Handoff: Capture real-device screenshots plus tactile feedback on footer reachability and event readability; adjust spacing again if thumbs still feel cramped.
Handoff: Validate the latest footer button offset and event padding on physical devices, gather feedback, and note any follow-up spacing tweaks before the next session.

** Session: Mobile refocus to Today
Time: Late evening
Task: Snap returning mobile sessions back to the current day after idle stretches

Accomplished:
- Added a mobile-only refocus guard in `CalendarContext` that tracks when the tab was hidden and auto-scrolls to today after 5+ minutes away.
- Reused the existing scroll helper so the virtualization layer stays in sync and preserves today’s styling invariants.
- Left desktop behaviour unchanged to avoid jarring jumps when returning on larger screens.

Energy Level: Steady
Next Step: QA on physical iOS/Android devices: background the app for 5+ minutes, reopen, confirm it recenters on today without breaking footer navigation.

* 2026-07-13: Define the shared multi-day event protocol
- Added the canonical first-day/last-day convention to [[file+emacs:/Users/jay/Dropbox/github/timeless/docs/instructions.org][docs/instructions.org]].
- Multi-day events use the full description and complete range on the first day and a ~Last day of [event]~ marker on the final day, with no intervening duplicates and no unsupported Emacs ~%%(diary-block ...)~ entries.
- Cross-referenced the protocol from [[file+emacs:/Users/jay/emacs/emacs-settings/docs/emacs-calendar-instructions.org][the Emacs Calendar guide]].

* 2026-07-13: Add revision-safe Dropbox synchronization
- Replaced unconditional Dropbox overwrites with revision-guarded ~update~ writes using the exact revision returned by the last load.
- Added durable browser tracking for the synchronized revision and unsynced local changes, plus serialized save/sync operations so rapid edits cannot race each other.
- A stale save now returns HTTP 409, preserves browser edits, and shows explicit ~Use Dropbox~ and ~Keep Browser~ recovery actions in the existing toast and command palette.
- Mirrored revision checks in local Vite middleware with content hashes and temporary-file replacement.
- Fixed the pre-existing Vite hook placement that caused the documented diary middleware URLs to fall through to ~index.html~.
- Added API and browser-storage regression coverage. Verification: 22 Vitest tests passed, the production build passed, a clean headless Chrome session loaded without console errors, and a forced stale-revision smoke test left [[file+emacs:/Users/jay/Dropbox/github/timeless/data/timeline.md][data/timeline.md]] unchanged.
