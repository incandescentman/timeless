...
Lessons Learned:
- Always check for native browser APIs before building custom solutions
- Modern browsers have solved most mobile input problems already
- The `<dialog>` element with showModal() is specifically designed for this use case
- Complex custom code often recreates browser behavior poorly
- When users say "isn't there a library for this?" - check native APIs first

Next Step:
- Remove original complex version after testing confirms simplified version works on all devices
...
Accomplished:
- Swapped the portal overlay for an HTML `<dialog>` in `MobileEventComposer.jsx`, using modal semantics to manage backdrop and focus trapping
