...
- Core components: App.tsx (state), Calendar.tsx (year grid), EventCard.tsx, EventForm.tsx
...
** Year Grid Rendering (Calendar.tsx)

Renders all 12 months in a responsive grid.

For each month:
1. Calculate first day of month
2. Generate day cells (including empty cells for alignment)
3. Check if each day isToday
4. Render events for each day
