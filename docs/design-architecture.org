...
- `DayCell.jsx` owns touch gesture handling for individual notes via a lightweight state machine in `DayEventRow`.
- Horizontal intent is detected only after surpassing `HORIZONTAL_ACTIVATION_THRESHOLD` and while vertical drift stays below `VERTICAL_ABORT_THRESHOLD`, so casual scrolling remains unaffected.
- Swiping **right** ≥ `SWIPE_DELETE_THRESHOLD` immediately removes the event (mirrors desktop delete shortcut). Swiping **left** ≥ `SWIPE_EDIT_THRESHOLD` toggles the inline editor for that note.
- After either gesture, click events are suppressed until the next interaction to avoid instant re-open or accidental edits.
...
- If thresholds ever change, update the shared constants near the top of `DayCell.jsx` and re-validate on real devices to keep gestures consistent and discoverable.
