...
** Fix
   ...
   - Rebuilt the composer around the native `<dialog>` element, letting the browser manage backdrop, focus trapping, and cancel semantics instead of our custom portal overlay, while falling back to the legacy overlay on browsers without `<dialog>` support.
...
* iOS Accessory Checkmark Drops Composer
  Date: 2025-10-13

** Symptom
   On mobile Safari the composer behaved while typing, but tapping the keyboard accessory ✓ saved and then left the modal stranded at the bottom of the screen. The UI froze until a full page refresh.

** Root Cause
   The accessory checkmark dismisses the keyboard without firing a click on our buttons. The input blurred, the keyboard closed, and React re-rendered before we cleared the sheet. Without an explicit intent the blur auto-cancelled, but the overlay was already mid-transition so we ended up with a half-open zombie modal.

** Fix
   - Tracked close intent with refs so we know whether the user meant to cancel, save, or simply lost focus.
   - Deferred blur auto-save/auto-cancel with a short timeout to let the button handlers mark their intent first.
   - Locked body scroll during the overlay and anchored it to `visualViewport` so iOS no longer repositions the sheet mid-close.

** Prevention Checklist
   - [ ] Treat blur as advisory only on mobile: always record the user’s intent before dismissing the keyboard.
   - [ ] When the keyboard accessory can dismiss inputs, verify both taps and hardware Return/Done paths on-device.
   - [ ] Keep the composer outside transformed containers so React state drives close animations instead of CSS hacks.
...
* Vite Dev Middleware Must Be Registered as a Plugin Hook

** Trigger
The documented ~/__load-calendar-diary~ endpoint returned ~index.html~ instead of JSON even though ~registerCalendarDiaryMiddleware~ existed in [[file+emacs:/Users/jay/Dropbox/github/timeless/vite.config.js][vite.config.js]].

** Mechanism
~configureServer~ is a Vite plugin hook, not a property of the top-level ~server~ options object. Putting it under ~server.configureServer~ leaves the function present in source but never registers it, so Vite’s SPA fallback handles the request and returns the app HTML.

** Actionable Insight
Wrap custom dev middleware in a plugin object and put it in ~plugins~:
#+begin_src javascript
...
After changing this hook, verify the endpoint itself with ~curl~ and confirm its content type/body; a successful Vite page load does not prove custom middleware is registered.
