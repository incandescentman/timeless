...
** The Fix
   ...
   - Rewrote the loaders to use functional `setWeeks` updates (lines 77-117), guaranteeing access to the latest state even under rapid callbacks.
   - Updated the observer effect to depend on the memoised loaders so the guard stays in sync.
