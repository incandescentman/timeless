...
** Architectural Notes
   ...
   - Loading previous months now compensates scroll with `window.scrollBy(diff)` (see `loadPreviousMonths`); keep that adjustment if you tweak markup so users stay anchored.
