...
* Overview
Timeless is an infinite scroll calendar application built with React. It provides seamless navigation through weeks, months, and years with persistent note-taking capabilities. The calendar stores data locally and, when configured, syncs to the Dropbox-backed [[file+emacs:/Users/jay/Dropbox/github/timeless/data/timeline.md][timeline.md]] diary so every device shares the same human-readable log.
...
1. Navigate to the project directory:
   #+BEGIN_SRC bash
   cd /Users/jay/Dropbox/github/timeless
   #+END_SRC
...
* Access Modes
- **Local Development**: Run =npm run dev= for full editing capabilities with hot reload. This project configures Vite on port 5000. The dev server exposes =/__load-calendar-diary= and =/__update-calendar-diary= so saves go straight into the regular Dropbox-synced file at [[file+emacs:/Users/jay/Dropbox/github/timeless/data/timeline.md][data/timeline.md]].
...
- ~/api/calendar~ saves accept JSON payloads, convert them to Markdown, and conditionally update the Dropbox diary only when the client’s ~baseRevision~ still matches Dropbox’s current revision.
- Each successful load returns ~serverRevision~ and ~serverFileExists~. Each save sends that baseline back; a stale revision returns HTTP 409 instead of overwriting newer Dropbox content.
- The browser records whether it has unsynced local edits. If Dropbox and the browser both changed, Timeless preserves the browser copy and offers explicit ~Use Dropbox~ and ~Keep Browser~ choices.
- ~lastSavedTimestamp~ and Dropbox’s ~server_modified~ time remain available for display and diagnostics. A browser created before revision tracking adopts the Dropbox revision only when the calendar content matches; divergent copies become an explicit conflict rather than trusting device clocks.
- Dev middleware uses a content hash as its local revision and replaces [[file+emacs:/Users/jay/Dropbox/github/timeless/data/timeline.md][data/timeline.md]] through a temporary file after a second revision check.
...
** Multi-Day Event Protocol
The shared Markdown format has no native date-range primitive. Represent a multi-day event with two boundary entries:
- On the first day, add the full event description, including its complete date range.
- On the final day, add ~Last day of [event]~.
- Do not repeat the event on intervening days.
- Do not use Emacs ~%%(diary-block ...)~ entries for shared Timeline data; Timeless and the custom Emacs agenda do not parse them.

This convention marks the event's opening and closing dates without cluttering every day in between.
...
- **Sync conflict**: Both copies changed after the browser’s last successful sync. Use the toast or command palette to choose ~Use Dropbox~ or ~Keep Browser~; Timeless never resolves this by silently overwriting one copy.
...
- [[file+emacs:/Users/jay/Dropbox/github/timeless/README.org][README.org]] - Feature overview and recent updates
- [[file+emacs:/Users/jay/Dropbox/github/timeless/docs/design-architecture.org][docs/design-architecture.org]] - Detailed component architecture
- [[file+emacs:/Users/jay/Dropbox/github/timeless/docs/codebase-wisdom.org][docs/codebase-wisdom.org]] - Development patterns and decisions
- [[file+emacs:/Users/jay/Dropbox/github/timeless/docs/experimental-mode-plan.org][docs/experimental-mode-plan.org]] - Future feature variants
